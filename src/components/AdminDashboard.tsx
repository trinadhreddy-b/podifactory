...
export const AdminDashboard: React.FC = () => {
  ...
  // New Podi Form state
  const [newPodiForm, setNewPodiForm] = useState<Omit<PodiItem, 'id'>>({
    name: '',
    teluguName: '',
    tagline: '',
    description: '',
    ...
    ingredients: ['Guntur Chillies', 'Roasted Lentils', 'Garlic', 'Cumin', 'Rock Salt'],
    healthBenefits: ['Authentic stone-ground recipe', 'Zero preservatives'],
    servingSuggestions: ['Hot rice with ghee', 'Idli and Dosa'],
    inStock: true,
    badge: 'New Blend',
    isFeatured: false,
  });
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 overflow-y-auto">
        ...
        >
          ...
          <div className="p-4 sm:p-5 border-b border-brand-outline-variant/30 flex items-center justify-between bg-brand-surface-container/70 shrink-0">
          ...
          {editingPodi && (
            <div className="fixed inset-0 z-60 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
              <div className="bg-brand-surface-card rounded-2xl max-w-lg w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto border border-brand-outline-variant shadow-2xl">
                <div className="flex justify-between items-center border-b border-brand-outline-variant/30 pb-3">
                  ...
                  <button onClick={() => setEditingPodi(null)}>
                    <X className="w-5 h-5 text-brand-muted" />
                  </button>
                </div>

                <form onSubmit={handleSaveEditedPodi} className="space-y-3 text-xs">
                  ...
                  <div className="space-y-1">
                    <label className="font-bold text-brand-on-surface">Telugu Sub-name:</label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-1">
                    <label className="font-bold text-brand-on-surface">Tagline / Short Subtitle:</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="font-bold text-brand-on-surface">Detailed Description:</label>
                    <textarea
                      rows={3}
                  ...
                  <div className="space-y-1">
                    <label className="font-bold text-brand-on-surface">Image URL:</label>
                    <input
                      type="url"
                  ...
                  <div className="flex justify-end gap-2 pt-3">
                    ...
                    <button
                      type="submit"
                      className="px-5 py-2 bg-brand-primary text-white rounded font-bold"
                      style={{ backgroundColor: 'var(--color-primary)' }}
                    >
                      Save Changes
                    </button>
          ...
          {isAddingNew && (
            <div className="fixed inset-0 z-60 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
              <div className="bg-brand-surface-card rounded-2xl max-w-lg w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto border border-brand-outline-variant shadow-2xl">
                ...
                <form onSubmit={handleCreateNewPodi} className="space-y-3 text-xs">
                  <div className="space-y-1">
                    <label className="font-bold text-brand-on-surface">Podi Name *:</label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      type="text"
                      required
                      placeholder="e.g. Fresh coriander leaves stone-ground with garlic"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="font-bold text-brand-on-surface">Base Jar Price (₹) *:</label>
                      <input
                        type="number"
                        required
                        value={newPodiForm.price}
                        onChange={(e) => {
                          ...
                          setNewPodiForm({
                            ...
                            price: val,
                            weights: [
                              { label: '100g Pouch', grams: 100, price: Math.round(val * 0.55) },
                              { label: '200g Jar', grams: 200, price: val },
                              { label: '500g Value Pack', grams: 500, price: Math.round(val * 2.25) },
                            ],
                          });
                        }}
                        className="w-full p-2 border border-brand-outline-variant rounded bg-brand-surface-card"
                  ...
                  <div className="space-y-1">
                    <label className="font-bold text-brand-on-surface">Description:</label>
                    <textarea
                      rows={3}
                      placeholder="Artisanal preparation, roasting method, and spice notes..."
